import type { Tool } from '@/types/tool';
import { ROUTES } from '@/constants/routes';

export const tools: Tool[] = [
  {
    id: 'json-formatter',
    name: 'JSON 格式化',
    description: '格式化、校验、转换 JSON 数据，支持树形视图',
    category: 'developer',
    icon: 'Braces',
    route: ROUTES.TOOLS_JSON,
    tags: ['json', 'format', 'validate', 'tree'],
    isHot: true,
    status: 'stable',
  },
  {
    id: 'regex-tester',
    name: '正则测试',
    description: '实时可视化正则匹配，支持分组捕获和常用模板',
    category: 'developer',
    icon: 'Regex',
    route: ROUTES.TOOLS_REGEX,
    tags: ['regex', 'pattern', 'match', 'test'],
    isHot: true,
    status: 'stable',
  },
  {
    id: 'ai-chat',
    name: 'AI 聊天助手',
    description: '智能对话助手，支持流式响应和提示词模板',
    category: 'ai',
    icon: 'Bot',
    route: ROUTES.TOOLS_AI_CHAT,
    tags: ['ai', 'chat', 'claude', 'assistant'],
    isHot: true,
    isNew: true,
    status: 'stable',
  },
  {
    id: 'markdown-editor',
    name: 'Markdown 编辑器',
    description: '实时预览的 Markdown 编辑器，支持导出',
    category: 'document',
    icon: 'FileText',
    route: ROUTES.TOOLS_MARKDOWN,
    tags: ['markdown', 'editor', 'preview', 'export'],
    status: 'stable',
  },
  {
    id: 'base64',
    name: 'Base64 编解码',
    description: 'Base64 编码与解码，支持 Unicode 字符',
    category: 'developer',
    icon: 'Binary',
    route: ROUTES.TOOLS_BASE64,
    tags: ['base64', 'encode', 'decode', 'convert'],
    isNew: true,
    status: 'stable',
  },
  {
    id: 'code-review',
    name: 'AI 代码审查',
    description: '智能代码质量分析，问题检测与优化建议',
    category: 'ai',
    icon: 'ScanSearch',
    route: ROUTES.TOOLS_CODE_REVIEW,
    tags: ['code', 'review', 'analysis', 'quality', 'security'],
    isHot: true,
    isNew: true,
    status: 'stable',
  },
];
